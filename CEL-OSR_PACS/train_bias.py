...
class ModelBaseline(object):
    ...
    def setup(self, flags):
        ...
        if flags.model == 'resnext':
            self.f_net = resnet18(num_classes=class_num)
    ...
    def setup_path(self, flags):
        ...
        root_folder = '../data'
        Data = Tiny_ImageNet_OSR(known=known, dataroot='../data', batch_size=16, img_size=224)
    ...
    def update_f(self,x,y,epoch,outer_criterion):
        ...
        f_loss_cls = edl.edl_digamma_loss(f_logits,y_onehat,epoch,class_num,50)
    ...
    def update_g(self,x,y,epoch,inner_criterion):
        ...
        y_onehat = edl.one_hot_embedding(y.long(), class_num)
        g_loss_cls = edl.edl_digamma_loss(g_logits,y_onehat,epoch,class_num,50)
    ...
    def test_workflow(self, epoch, flags):
        ...
        accuracies = []
        # train_loader = self.train_data.train_loader
        # val_loader = self.train_data.test_loader
        open_loader = self.train_data.out_loader
        # print(len(self.val_loader))
        print('val_data_close:', len(self.val_loader.dataset))
        print('val_data_open:', len(open_loader.dataset))
        for i, (image_test, label_test) in enumerate(self.val_loader):
            image_test, label_test = image_test.to(device), label_test.to(device)
            outputs, _ = self.f_net(image_test)
            predictions = outputs.data.max(1)[1]
            total = label_test.size(0)
            correct = (predictions == label_test.data).sum()
            acc = float(correct) * 100. / float(total)
