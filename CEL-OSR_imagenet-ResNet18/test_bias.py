...
train_data = Data
model =resnet18()
model.load_state_dict(torch.load('./imagenet/models/best_model.tar')['state'])
model.eval()
model = model.cuda()
...
for i in val_set.image:
    j = 0
    image = np.array(i)
    noise = np.random.randn(image.shape[0], image.shape[1], image.shape[2]) * 0.1 
...
for i in val_set.image:
    j = 0
    image = np.array(i)
    noise = np.random.randn(image.shape[0], image.shape[1], image.shape[2]) * 0.2 
    noise_image = image + noise
    # val_set.image[j]=noise_image
    noise_image = noise_image.clip(min=0, max=1)
    noise_image = noise_image.astype(np.float32)
