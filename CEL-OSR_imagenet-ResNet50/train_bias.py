...
import edl
from models.imagenet_models import resnet50, resnet18, bagnet18
from models.bagnet import bagnet9
...
class ModelBaseline(object):
    ...
    def setup(self, flags):
        ...
        if flags.model == 'resnext':
            # self.f_net = resnet18(feature_pos='post',num_classes=class_num,pretrained=False)
            # self.g_net = bagnet18(feature_pos='post',num_classes=class_num)
            self.f_net = resnet50(num_classes=class_num,pretrained=True)
            self.g_net = bagnet9(pretrained='imagenet')
            # num_ftrs = self.f_net.fc.in_features
            # self.f_net.fc = torch.nn.Linear(num_ftrs,class_num)
            num_ftrs = self.g_net.last_linear.in_features
            self.g_net.last_linear = torch.nn.Linear(num_ftrs,class_num)
    ...
    def setup_path(self, flags):
        ...
        root_folder = '../data'
        Data = Tiny_ImageNet_OSR(known=known, dataroot='../data', batch_size=flags.batch_size, img_size=64)
    ...
    def val_workflow(self, epoch, flags):
        ...
        open_loader = self.train_data.out_loader
        print('val_data_close:', len(val_loader.dataset))
        print('val_data_open:', len(open_loader.dataset))
        acc, auc, _, _ = evaluation(self.f_net, val_loader, open_loader)
        # acc, auc, f1 = evaluation2(model, val_loader, val_loader_open)
...
class ModelCELOSR(ModelBaseline):
    ...
    def maximize(self, flags):
        ...
        #     num_workers=flags.num_workers,
        #     pin_memory=True)
        # Data = Tiny_ImageNet_OSR(known=known, dataroot='../data', batch_size=32, img_size=64)
        # self.train_loader = Data.train_loader
        # self.valloader = Data.val_loader
        # self.openloader = Data.out_loader
        images, labels = [], []
